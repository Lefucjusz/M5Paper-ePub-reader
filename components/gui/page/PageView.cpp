...
namespace gui
{
    namespace
    {
        ...
        auto turnPage(PageDirection direction)
        {
            switch (direction) {
        ...
        auto addBlockToPage(lv_obj_t *page, const TextBlock &textBlock, std::size_t blockOffsetBytes)
        {
            /* Add new block */
        ...
        auto getFirstExcessCharIndex(const lv_obj_t *block) -> std::size_t
        {
            ...
            if (blockEndY <= GUI_MAIN_AREA_MAX_Y) {
                return noExcessChar;
            }

            /* The text starts after the end of main area */
            if (blockStartY >= GUI_MAIN_AREA_MAX_Y) {
                return 0;
            }
            ...
            lv_point_t firstExcessPointCoords = {
                .x = 0,
                .y = static_cast<lv_coord_t>(GUI_MAIN_AREA_MAX_Y - blockStartY)
            };
            ...
            while (true) {
                ...
                if (blockBytesLeft == 0) {
                    ...
                    if (isSectionEnd(section, blockIndex)) {
                        ESP_LOGI(TAG, "Reached end of section"); //%zu", ctx.spine_index); // TODO
                        break;
                    }
                    else {
                        blockOffsetBytes = 0;
    ...
    auto pageViewCreate(const Epub *epub, const std::filesystem::path &sectionPath) -> void
    {
        /* Sanity check */
        if (epub == nullptr) {
            return;
        ...
        auto section = epub->getSection(sectionPath);
